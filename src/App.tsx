...
import { WelcomeDialog, CreateRoomDialog, JoinRoomDialog, GlobalToast, SystemNotificationBubbles } from './components/Dialogs';

// ================= Lobby Sub-components =================
const LobbyHeader: React.FC<{ userName: string; onEditName: () => void; onCreateRoom: () => void }> = ({ userName, onEditName, onCreateRoom }) => (
    ...
    <div className="flex items-center gap-3 sm:gap-5 w-full sm:w-auto justify-between sm:justify-end">
      <div className="flex items-center gap-3 pl-4 pr-1.5 py-1.5 bg-zinc-900/50 rounded-2xl border border-zinc-800/80 shadow-inner backdrop-blur-md">
        <div className="flex flex-col items-end">
          ...
          <span className="text-sm font-bold text-zinc-200 max-w-[100px] sm:max-w-none truncate leading-none">{userName || '未登录'}</span>
        </div>
        <button onClick={onEditName} className="w-10 h-10 flex items-center justify-center btn-ghost border border-zinc-700/30" title="修改昵称">
          <Edit2 className="w-4 h-4" />
        </button>
      </div>
      <button onClick={onCreateRoom} className="group relative btn-primary h-12 px-6 overflow-hidden flex items-center gap-2">
      ...
      <div className="relative z-10 h-full flex flex-col p-5">
        <div className="flex items-center justify-between mb-4">
          <div className="flex-1 min-w-0">
            <h3 className="text-xl font-black tracking-tight text-white group-hover:text-emerald-400 transition-colors truncate">{room.name}</h3>
            <div className="flex items-center gap-2 mt-1">
              <span className="text-[10px] font-bold uppercase tracking-tighter text-zinc-500 bg-zinc-800/50 px-2 py-0.5 rounded-full border border-zinc-700/30">{room.hasPassword ? '私密' : '公开'}</span>
        ...
        <div className="flex-1 flex items-center gap-4 py-4 px-3 bg-white/5 rounded-2xl shadow-inner">
          ...
          <div className="flex-1 min-w-0">
            {room.currentSong ? (
              ...
              <div className="flex flex-col opacity-40"><Music className="w-5 h-5 mb-1 text-zinc-400" /><span className="text-xs font-bold text-zinc-400 tracking-tight">暂无内容</span></div>
            )}
          </div>
        </div>
        <div className="mt-5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="relative w-8 h-8 rounded-xl bg-gradient-to-br from-zinc-700 to-zinc-900 flex items-center justify-center text-[10px] font-black text-zinc-300 border border-white/5">{room.hostName.charAt(0).toUpperCase()}</div>
            <div className="flex flex-col"><span className="text-[9px] font-bold text-zinc-500 uppercase tracking-tighter">房主</span><span className="text-xs font-bold text-zinc-300">{room.hostName}</span></div>
          </div>
          <div className="px-3 py-1.5 bg-emerald-500/10 border border-emerald-500/20 rounded-xl flex items-center gap-2"><Users className="w-3 h-3 text-emerald-400" /><span className="text-xs font-black text-emerald-400">{room.usersCount}</span></div>
        </div>
      </div>
    </motion.div>
  );
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-50 font-sans selection:bg-emerald-500/30 relative overflow-hidden">
      ...
      <WelcomeDialog isOpen={showWelcome} onClose={() => setShowWelcome(false)} initialName={welcomeForceInitial} />
      <JoinRoomDialog targetRoom={joinRoomTarget} onClose={() => setJoinRoomTarget(null)} onJoinSuccess={() => setJoinRoomTarget(null)} />
    </div>
  );
}
