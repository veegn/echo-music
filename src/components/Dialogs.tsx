...
export const WelcomeDialog: React.FC<{ isOpen: boolean; onClose: () => void; initialName?: string }> = ({
    isOpen,
    onClose,
    initialName,
}) => {
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} zIndex="z-[60]" width="max-w-sm">
            ...
            <h2 className="text-xl font-bold mb-2 text-center">{userName ? '修改昵称' : '欢迎来到 Echo Music'}</h2>
            <form onSubmit={handleSubmit} className="text-center">
                <input
                    ...
                    autoFocus
                    required
                    className="input-dark w-full text-center mb-4"
                />
                <div className="flex gap-3">
                    {userName && (
                        <button type="button" onClick={onClose} className="flex-1 px-4 py-3 text-sm text-zinc-400">
                            取消
                        </button>
                    )}
...
export const JoinRoomDialog: React.FC<{ targetRoom: any; onClose: () => void; onJoinSuccess: () => void }> = ({
    targetRoom,
    onClose,
    onJoinSuccess,
}) => {
    ...
    const handleJoin = async (e: React.FormEvent) => {
        ...
        try {
            ...
            showToast(e.message === 'Incorrect password' ? '房间密码错误' : '加入房间失败', 'error');
        } finally {
            setLoading(false);
        }
    };
        ...
        <Modal isOpen={!!targetRoom} onClose={onClose} width="max-w-sm">
            ...
            <form onSubmit={handleJoin} className="space-y-4">
                <input
                    ...
                    autoFocus
                    placeholder="请输入房间密码"
                    className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-4 py-3 text-sm text-center tracking-widest"
                />
                <div className="flex gap-3">
                    <button type="button" onClick={onClose} className="flex-1 text-zinc-400">
                        取消
                    </button>
                    <button type="submit" disabled={loading} className="flex-1 bg-emerald-500 text-zinc-950 py-2.5 rounded-lg font-bold">
                        {loading ? <Loader2 className="w-4 h-4 animate-spin mx-auto" /> : '加入'}
                    </button>
                </div>
            </form>
        </Modal>
            ...
            {tab === 'manual' ? (
                <form
                    onSubmit={(e) => {
                        e.preventDefault();
                        setCookie(input);
                        localStorage.setItem(COOKIE_STORAGE_KEY, input);
                        onClose();
