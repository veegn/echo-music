...
export default function MusicPanel({ isHost }: { isHost: boolean }) {
  ...
  const renderTrackList = (tracks: any[], showPlayAll = false) => (
    <div className="space-y-1">
      ...
      {tracks.map((song) => (
        <div key={song.songmid || song.id} className="group flex items-center justify-between p-2.5 sm:p-3 list-item-hover">
                      ...
                      <div className="text-[9px] sm:text-[10px] text-zinc-500 shrink-0 flex flex-col items-end gap-0.5">
                        ...
                        {isHost && (
                          <div className="flex items-center gap-0.5 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                            <button onClick={() => reorderQueue(index, Math.max(0, index - 1))} disabled={index === 0} className="p-0.5 hover:bg-zinc-700 rounded disabled:opacity-30">
                              <ArrowUp className="w-2.5 h-2.5 sm:w-3 sm:h-3" />
                            </button>
                            <button onClick={() => reorderQueue(index, Math.min(room.queue.length - 1, index + 1))} disabled={index === room.queue.length - 1} className="p-0.5 hover:bg-zinc-700 rounded disabled:opacity-30">
                              <ArrowDown className="w-2.5 h-2.5 sm:w-3 sm:h-3" />
                            </button>
                            <button onClick={() => removeSong(index)} className="p-0.5 hover:bg-red-500/20 text-red-400 rounded">
